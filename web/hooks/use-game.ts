...
export const useGame = (options: UseGameOptions = {}) => {
  ...
  useEffect(() => {
    ...
    const handleGameStarted = (data: any) => {
      ...
      // Just update state - backend only emits this after all on-chain transactions are confirmed
      setGameState(prev => ({
        ...prev,
        gameStatus: 'playing',
    ...
    const handleGameStartedPreTx = async (data: any) => {
      ...
      const gameStateData = data.gameState || data
      const myPlayerId = publicKey?.toString().slice(0, 8)
      const gameId = gameStateData.gameId
      ...
      if (currency === 'sol' && publicKey && gameStateData.gameType === 'private') {
        ...
        try {
          ...
          if (isPlayer1) {
            ...
            console.log('📤 Emitted onchain_game_created event')
          } else if (isPlayer2) {
            ...
            console.log('📤 Emitted onchain_game_joined event')
          }
      ...
      setTimeout(() => {
        ...
        setGameState(prev => {
          ...
          const newState = {
            ...
            // Set ready: true to allow players to make moves in the new round
            player1: {
              ...prev.player1,
              currentMove: null,
              ready: true,
