...
pub mod rps_game {
    ...
    pub fn finalize_game(ctx: Context<FinalizeGame>, _game_id: String) -> Result<()> {
        ...
        if winner_key == ctx.accounts.game.player1 {
            ...
            if ctx.accounts.game.currency_type == CurrencyType::Points {
                ...
                let winner_amount = total_pot - (total_pot * fee_rate / 10000); // fee_rate is in basis points
                
                // Calculate platform fee amount
                let platform_fee_amount = total_pot * fee_rate / 10000;
                ...
                **ctx.accounts.platform_wallet.to_account_info().try_borrow_mut_lamports()? += platform_fee_amount;
                ...
                msg!("Platform fee: {} lamports ({} SOL) transferred to platform wallet {}", 
                    platform_fee_amount,
                    platform_fee_amount as f64 / anchor_lang::solana_program::native_token::LAMPORTS_PER_SOL as f64,
...
pub struct InitializeUserProfile<'info> {
    #[account(
        init,
        payer = user,
        space = 8 + 32 + 8 + 4 + 4 + 4 + 8 + 8, // discriminator + profile data
